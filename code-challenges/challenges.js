...
function charCount(str) {
  str = str.split('');
  let letterCount = { };
  // iterating for each letter in string
  for (let i = 0; i < str.length; i++) {
    let letter = str[i];
    // if letter is a key in the object, letterCount, 
    if (letterCount.hasOwnProperty(`${letter}`)) {
      //     add +1 to that key's value in the object
      letterCount[`${letter}`] += 1; 
    } else {
      // else, add the letter as a key in the object, and with a value of one
      letterCount[`${letter}`] = 1; 
    }
  }
  return letterCount;
